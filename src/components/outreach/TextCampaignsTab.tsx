...
import { Download, Eye, Save, ExternalLink, Users } from "lucide-react";
...
export default function TextCampaignsTab() {
  ...
  useEffect(() => { fetchVoters(); }, [targetCity, targetParty]);

  const fetchFilters = async () => {
    const { data } = await supabase.from("voters").select("city, party");
    if (data) {
      setCities([...new Set(data.map(v => v.city).filter(Boolean))].sort());
      setParties([...new Set(data.map(v => v.party).filter(Boolean))].sort());
    }
  };
  ...
  const previewMessage = (voter: Voter) => script.replace(/\{\{first_name\}\}/g, voter.first_name || "Friend");
  ...
  const handleSave = async () => {
    ...
    const { error } = await (supabase.from as any)("texting_campaigns").insert({
      user_id: user.id, name, script_template: script,
      ...
      <div className="space-y-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <Textarea value={script} onChange={e => setScript(e.target.value)} rows={4} placeholder="Hi {{first_name}}, this is..." />
          </CardContent>
        </Card>
